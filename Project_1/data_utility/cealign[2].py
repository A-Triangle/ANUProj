...
def c_alpha_rmsd(model_ref, reference_filename, model_sample, sample_filename):
    ...
    for chain_ref in model_ref:
        for resid_ref in chain_ref:
            ...
            numbered = resid_ref.id[1] - model_ref.child_list[0].child_list[0].id[1]
    ...
    while j < len(reference_data) :
        ...
        comparison_CA = (reference_data[0]['residue_CA'] - sample_data[j]['residue_CA']) 
    ...
    while i < len(reference_data)-10:
        
        comparison_CA = (reference_data[i]['residue_CA'] - sample_data[init_id]['residue_CA']) 
        magnitude_comparison_CA=norm(comparison_CA, 2)
        print(magnitude_comparison_CA)
        ...
        current = sample_data[init_id]['residue_id'] 
        next = reference_data[i+1]['residue_id']
        
        int(current); int(next)
        
        step = next - current
        print(step, next, current)
